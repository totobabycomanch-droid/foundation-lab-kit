<template>
  <div class="company-layout">
    ...
    <main class="main-content" aria-labelledby="supplier-master-title">
      <header class="content-header">
        <h1 id="supplier-master-title">공급사 등록 관리</h1>
        <div class="header-breadcrumb">기준정보관리 > 공급사등록(A112)</div>
      </header>
      ...
      <section class="grid-container">
        ...
        <div class="grid-wrap list-grid">
          <div class="grid-header-actions">
            <div class="search-box">
              <label class="sr-only" for="supplier-search">공급사명 또는 업체 ID 검색</label>
              <input id="supplier-search" type="search" v-model="searchQuery" placeholder="공급사명/ID 검색" :disabled="isLoading" @keyup.enter="fetchSuppliers">
              <button type="button" class="btn-search" :disabled="isLoading" @click="fetchSuppliers">{{ isLoading ? '조회 중...' : '조회' }}</button>
            </div>
            <div class="button-group">
              <button type="button" class="btn-add" :disabled="isSaving" @click="addNewSupplier">신규 등록</button>
            </div>
          </div>
          <p v-if="isLoading" class="sr-only" role="status">공급사 목록을 불러오는 중입니다.</p>
          <div class="table-scroll">
            <table class="data-table" :aria-busy="isLoading">
              <caption class="sr-only">공급사 목록</caption>
              <thead>
                <tr>
                  <th>업체 ID</th>
                  <th>공급사명</th>
              ...
              <tbody>
                <tr 
                  v-for="corp in suppliers" 
                  :key="corp.corp_id"
                  :class="{ active: selectedCorpId === corp.corp_id }"
                  :aria-selected="selectedCorpId === corp.corp_id"
                  tabindex="0"
                  @click="selectSupplier(corp)"
                  @keydown.enter="selectSupplier(corp)"
                  @keydown.space.prevent="selectSupplier(corp)"
                >
                  <td>{{ corp.corp_id }}</td>
                  <td>{{ corp.name_kr }}</td>
                  <td>{{ corp.ceo_name }}</td>
                ...
                <tr v-if="!isLoading && suppliers.length === 0">
                  <td colspan="4" class="empty-cell">조회된 공급사가 없습니다.</td>
                </tr>
              </tbody>
            </table>
          </div>
        ...
        <div class="grid-wrap detail-grid">
           <div class="grid-header-actions">
            <h3>공급사 상세 정보</h3>
            <div class="button-group">
              <button type="button" class="btn-save" :disabled="isSaving" @click="saveSupplier">{{ isSaving ? '저장 중...' : '저장' }}</button>
            </div>
          </div>
          <div class="detail-form">
            <div class="form-row">
              <div class="form-item">
                <label for="supplier-category">업체 구분</label>
                <input id="supplier-category" type="text" value="공급사(Supplier)" readonly class="readonly">
              </div>
              <div class="form-item">
                <label for="supplier-id">업체 ID</label>
                <input id="supplier-id" type="text" v-model="detail.corp_id" required :readonly="!isNew" :class="{readonly: !isNew}">
              </div>
            </div>
            <div class="form-row">
              <div class="form-item">
                <label for="supplier-name">공급사명(KR)</label>
                <input id="supplier-name" type="text" v-model="detail.name_kr" required>
              </div>
              <div class="form-item">
                <label for="supplier-password">비밀번호</label>
                <input id="supplier-password" type="password" v-model="detail.password" :required="isNew" autocomplete="new-password" placeholder="신규 등록 또는 변경 시 입력">
              </div>
            </div>
            <div class="form-row">
              <div class="form-item">
                <label for="supplier-ceo">대표자명</label>
                <input id="supplier-ceo" type="text" v-model="detail.ceo_name">
              </div>
              <div class="form-item">
                <label for="supplier-tel">전화번호</label>
                <input id="supplier-tel" type="tel" v-model="detail.tel_no" required>
              </div>
            </div>
            <div class="form-row">
              <div class="form-item">
                <label for="supplier-zip">우편번호</label>
                <div class="input-with-btn">
                  <input id="supplier-zip" type="text" v-model="detail.zip_code" readonly class="readonly">
            ...
            <div class="form-row full">
              <div class="form-item">
                <label for="supplier-address1">주소</label>
                <input id="supplier-address1" type="text" v-model="detail.address1" placeholder="기본 주소" readonly class="readonly">
                <label class="sr-only" for="supplier-address2">상세 주소</label>
                <input id="supplier-address2" type="text" v-model="detail.address2" placeholder="상세 주소" class="address-detail">
              </div>
            </div>
          </div>
        </div>
...
const fetchSuppliers = async () => {
  ...
  try {
    ...
    if (!res.ok) throw new Error(await responseError(res, '공급사 목록을 불러오지 못했습니다.'));
    const data = await res.json();
    suppliers.value = Array.isArray(data) ? data : [];
  } catch (e) {
    suppliers.value = [];
    errorMessage.value = e.message || '공급사 목록을 불러오지 못했습니다.';
  } finally {
    isLoading.value = false;
  }
};

const selectSupplier = (corp) => {
...
const saveSupplier = async () => {
    ...
    if (!detail.value.corp_id || !detail.value.name_kr || !detail.value.tel_no) {
        errorMessage.value = '업체 ID, 공급사명과 전화번호를 입력해 주세요.';
        return;
    }
    ...
    try {
        ...
        if (payload.password) {
            ...
            // 수정 시 비밀번호가 없으면 기존 비밀번호 유지
            delete payload.password;
        }

        payload.corp_category = '30';
        isSaving.value = true;
        ...
        if (!res.ok) throw new Error(await responseError(res, '공급사 저장에 실패했습니다.'));
        const wasNew = isNew.value;
        if (wasNew) addNewSupplier();
        statusMessage.value = '공급사 정보를 저장했습니다.';
        await fetchSuppliers();
    } catch (e) {
        errorMessage.value = e.message || '공급사 저장에 실패했습니다.';
    } finally {
